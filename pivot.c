...
matrix_t* pivot_ge_matrix (matrix_t* a, int* row_per) {
        ...
        if (c != NULL) {
                ...
                for (k = 0; k < rn - 1; k++) {      /* eliminujemy (zerujemy) kolumnę nr k */
                        ...
                        for (i = k + 1; i < rn; i++)    if (fabs (*(e + i * cn + k)) > fabs (*(e + piv * cn + k)))      piv = i;
...
void pivot_ge_in_situ_matrix (matrix_t* c) {
        ...
        for (k = 0; k < rn - 1; k++) {          /* eliminujemy (zerujemy) kolumnę nr k */
                int piv = k; 
                                                /* wybór elementu dominującego - maks. z k-tej kol., poniżej diag */
                for (i = k + 1; i < rn; i++)    if (fabs (*(e + i * cn + k)) > fabs (*(e + piv * cn + k)))
