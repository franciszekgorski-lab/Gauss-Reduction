...
matrix_t* make_matrix (int rn, int cn) {
        ...
        if (( new_mat->e = malloc( (size_t)rn * (size_t)cn * sizeof(double)) ) == NULL) {
                free(new_mat);
                return NULL;
        }
        ...
        new_mat->rn = rn;
        new_mat->cn = cn;
        memset(new_mat->e, 0, (size_t)(rn * (size_t)cn * sizeof(double)));
        return new_mat;
}
...
double get_entry_matrix (matrix_t* m, int i, int j ) {
        if (i >= 0 && i < m->rn && j >= 0 && j <= m->cn)        return m->e[i * m->cn + j];
        else                                                    return -999;
}
...
matrix_t* mull_matrix (matrix_t* a, matrix_t* b) {
        if (a == NULL || b == NULL || a->cn != b->rn)   return NULL;
        else {
                ...
                for (i = 0; i < c->rn; i++) {
                        for (j = 0; j < c->cn; j++) {
                                double s = 0;
                                for (k = 0; k < a->cn; k++)     s += *(a->e + i * a->cn + k) * *(b->e + k * b->cn + j);
